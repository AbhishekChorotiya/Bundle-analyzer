...
AI-powered bundle analyzer specifically tailored for the **Hyperswitch Web SDK** (`orca-payment-page`) — a payment orchestration platform.
...
Zero npm dependencies. Clone this repo and create a `.env` file:

```bash
# .env
OPENAI_API_KEY=<your-api-key>
OPENAI_BASE_URL=<your-api-endpoint>
REPO_URL=<git-clone-url-for-hyperswitch-web>
```

The CLI and `run.sh` both automatically load `.env` at startup — no `dotenv` dependency required.
...
The `hyperswitch-web/` directory is a git submodule pointing to [juspay/hyperswitch-web](https://github.com/juspay/hyperswitch-web). It serves as a local codebase reference for analysis context.
...
The `run.sh` script handles the full workflow: clones fresh copies of base and PR branches, installs deps, builds production webpack stats, runs AI analysis, and generates all report formats into `reports/`.
...
./run.sh --pr feat/my-feature

# Compare specific base + PR branches
./run.sh --base develop --pr feat/x

# Force rebuild (skip cache)
./run.sh --force-build
```

Reports are generated into `reports/` (gitignored):
...
node cli.js --base develop --head feature/new-payment-method
...
node cli.js --comment-file bundle-report.md

# Or post directly to GitHub (requires GITHUB_TOKEN)
node cli.js --post-comment --pr 123
```
...
node scripts/build-stats.js --ref main --output main-stats.json
```
...
jobs:
  analyze:
    ...
    steps:
      ...
      - run: |
          npm ci
          node cli.js --base main --head HEAD
```
...
```bash
# Check webpack config exists in the target repo
...
├── README.md
├── hyperswitch-web/          # Git submodule — codebase reference
├── lib/
