 ...
 *
 * Usage:
 *   node cli.js --base main --head current    # Local mode with branch comparison
 *   node cli.js --base-stats base.json --pr-stats pr.json  # File mode
 */
...
MODES:
  ...
  Branch Mode (for local testing):
    node cli.js --base main --head my-feature

  File Mode (for CI):
    node cli.js --base-stats base.json --pr-stats pr.json
...
EXAMPLES:
  ...
  node cli.js --base develop --head feature/new-ui

  # CI mode with pre-built stats
  node cli.js --base-stats ./stats/base.json --pr-stats ./stats/pr.json

  # Dry run without AI (faster)
  node cli.js --skip-ai

  # Save JSON output
  node cli.js --json output.json
`);
}
