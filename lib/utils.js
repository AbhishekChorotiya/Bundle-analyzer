...
/**
 * Validate a git ref name (branch, tag, or commit hash) to prevent command injection.
 * Only allows safe characters: alphanumeric, hyphens, dots, underscores, slashes.
 * @param {string} ref - Git ref to validate
 * @returns {string} The ref if valid
 * @throws {Error} If ref contains unsafe characters
 */
function validateGitRef(ref) {
  if (!ref || typeof ref !== 'string') {
    throw new Error('Git ref must be a non-empty string');
  }
  // Only allow safe characters for git refs: alphanumeric, hyphens, dots, underscores, slashes
