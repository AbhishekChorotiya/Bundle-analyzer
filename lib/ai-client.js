...
6. Consider compressed sizes (gzip/brotli) when assessing real-world user impact. Raw sizes overstate the cost of text-based assets like JS and CSS.
7. Flag duplicate dependencies as a common source of bundle bloat. If the PR introduces new duplicates, suggest npm dedupe or package.json resolutions.
...
function buildAnalysisPrompt(diff, detections, context) {
  ...
  const promptParts = [
    ...
    '',
    '## Summary',
    `- Total Size Change: ${formatBytes(diff.totalAssetDiff || 0, { signed: true })} (${formatBytes(diff.prAssetSize || 0)} vs ${formatBytes(diff.baseAssetSize || 0)})`,
    `- node_modules Change: ${formatBytes(diff.nodeModulesDiff, { signed: true })}`,
  ...
  // Add asset-level (output file) changes
  const assetDiff = diff.assetDiff || [];
  const significantAssets = assetDiff.filter(a => a.type !== 'unchanged');
  if (significantAssets.length > 0) {
    promptParts.push('', '## Output File Changes (Post-Minification)');
    promptParts.push('These are the actual files served to users:');
    for (const asset of significantAssets.slice(0, 15)) {
      ...
      const label = asset.type === 'added' ? '[NEW]' : asset.type === 'removed' ? '[DEL]' : '[CHG]';
      promptParts.push(`- ${label} ${asset.name}: ${baseSizeStr} → ${prSizeStr} (${asset.changeFormatted})`);
      // Include top contributors (module-level reasons for size change)
      const reasons = asset.reasons || [];
      if (reasons.length > 0 && asset.type !== 'added' && asset.type !== 'removed') {
        const reasonStr = reasons.map(r => `${r.name} (${r.changeFormatted})`).join(', ');
        promptParts.push(`  Top contributors: ${reasonStr}`);
      }
    }
    if (diff.baseAssetSize || diff.prAssetSize) {
      promptParts.push(`- Total output: ${formatBytes(diff.baseAssetSize || 0)} → ${formatBytes(diff.prAssetSize || 0)}`);
    }
  }
  ...
  if (significantEntrypoints.length > 0) {
    promptParts.push('', '## Entrypoint Changes');
...
function generateFallbackAnalysis(diff, detections) {
  ...
  const significantIncrease = (diff.totalAssetDiff || diff.totalDiff) > 100 * 1024;
...
function computeFallbackConfidence(diff, detections, verdict) {
  const absDiff = Math.abs(diff.totalAssetDiff || diff.totalDiff);
  const hasCritical = detections.critical.length > 0;
  const hasWarnings = detections.warnings.length > 0;
