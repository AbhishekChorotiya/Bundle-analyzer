...
function runDetection(diff, options = {}) {
  ...
  violations.push(...detectNewEntrypoints(diff));
  violations.push(...detectLargeAssetChanges(diff));
...
function detectUnexpectedDependencies(diff) {
  ...
  // Merge with hardcoded list so it works both inside and outside the target repo
  ...
  try {
    ...
    if (fs.existsSync(pkgPath)) {
      ...
      // devDependencies like 'rescript' can emit runtime helpers (e.g.
      // caml_js_exceptions.js via @rescript/core) into the production bundle.
      // Not checking devDeps causes false-positive UNEXPECTED_DEPENDENCY warnings.
      const pkgDeps = [
        ...Object.keys(pkg.dependencies || {}),
        ...Object.keys(pkg.devDependencies || {}),
      ];
      for (const dep of pkgDeps) {
        if (!liveDeps.includes(dep)) liveDeps.push(dep);
      }
    }
  } catch {
...
/**
 * Detect new entrypoints being added.
 * A new entrypoint can significantly increase the deployment footprint.
 * @param {import('./diff-engine').BundleDiff} diff
 * @returns {RuleResult[]}
 */
function detectNewEntrypoints(diff) {
  const results = [];
  ...
  for (const ep of entrypointDiff) {
    if (ep.type === 'added') {
      results.push({
        ...
        message: `New entrypoint added: "${ep.name}" (${formatBytes(ep.prSize)})`,
        details: {
          entrypoint: ep.name,
          size: ep.prSize,
          sizeFormatted: formatBytes(ep.prSize),
          suggestion: `Review whether "${ep.name}" is intentional. New entrypoints add to the initial download set served to users.`,
        },
      });
    } else if (ep.type === 'removed') {
      results.push({
        id: 'REMOVED_ENTRYPOINT',
        severity: 'info',
        category: 'Architecture',
        message: `Entrypoint removed: "${ep.name}" (was ${formatBytes(ep.baseSize)})`,
        details: {
          entrypoint: ep.name,
          size: ep.baseSize,
          sizeFormatted: formatBytes(ep.baseSize),
        },
      });
    }
  }
...
/**
 * Detect individual assets with large size regressions.
 * Flags any single output file that grew by more than 50KB.
 * @param {import('./diff-engine').BundleDiff} diff
 * @returns {RuleResult[]}
 */
function detectLargeAssetChanges(diff) {
  const results = [];
  const assetDiff = diff.assetDiff || [];
  const threshold = THRESHOLDS.SINGLE_MODULE_INCREASE_KB * 1024; // Reuse 50KB threshold

  for (const asset of assetDiff) {
    if (asset.type !== 'changed') continue;
    if (asset.change <= threshold) continue;

    results.push({
      id: 'LARGE_ASSET_INCREASE',
      severity: 'warning',
      category: 'Asset Size',
      message: `Output file grew significantly: ${asset.name} (+${formatBytes(asset.change)})`,
      details: {
        asset: asset.name,
        baseSize: asset.baseSize,
        prSize: asset.prSize,
        change: asset.change,
        changeFormatted: formatBytes(asset.change),
        suggestion: `The output file "${asset.name}" grew by ${formatBytes(asset.change)}. Investigate which modules contribute to this growth using the module-level diff.`,
      },
    });
  }
...
module.exports = {
  ...
  detectNewEntrypoints,
  detectLargeAssetChanges,
  THRESHOLDS,
