...
function parseStats(stats) {
  ...
  // Build chunk ID → name lookup so modules get stable chunk names
  const chunkIdToName = buildChunkIdToNameMap(stats);

  const modules = stats.modules
    .filter(m => !nestedNames.has(m.name))
    .map(m => parseModule(m, chunkIdToName));
  ...
  const chunkNameToAssets = buildChunkNameToAssetsMap(stats);
 ...
 * @param {Object.<number, string>} [chunkIdToName] - Optional chunk ID → name lookup
 * @returns {ParsedModule}
 */
function parseModule(module, chunkIdToName) {
  ...
  const chunks = Array.isArray(module.chunks) ? module.chunks : [];

  // Resolve chunk IDs to stable chunk names when lookup is available
  let chunkNames = [];
  if (chunkIdToName) {
    chunkNames = chunks
      .map(cid => chunkIdToName[cid])
      .filter(Boolean);
  }
...
function cleanModuleName(name) {
  ...
  return name
    // Strip loader prefixes: "style-loader!css-loader?modules!./src/App.css" → "./src/App.css"
    .replace(/^.*!/, '')
    // Strip query strings: "./src/App.js?v=1" → "./src/App.js"
    .replace(/\?[^/]*$/, '')
    // Strip concatenated module suffixes: "./src/App.js + 5 modules" → "./src/App.js"
...
/**
 * Build a map from chunk name → asset file names.
 * Unlike numeric chunk IDs, chunk names (e.g. "main", "vendor") are semantically
 * stable across webpack builds, making them safe for cross-build comparisons.
 * @param {Object} stats - Raw webpack stats
 * @returns {Object.<string, string[]>}
 */
function buildChunkNameToAssetsMap(stats) {
  const map = {};
  if (!stats.chunks || !Array.isArray(stats.chunks)) return map;

  for (const chunk of stats.chunks) {
    if (Array.isArray(chunk.names) && Array.isArray(chunk.files)) {
      const deliverableFiles = chunk.files.filter(f => isDeliverableAsset(f));
      for (const name of chunk.names) {
        if (!map[name]) map[name] = [];
        // Merge without duplicates
        for (const file of deliverableFiles) {
          if (!map[name].includes(file)) map[name].push(file);
        }
      }
    }
  }
...
/**
 * Build a map from chunk numeric ID → chunk name.
 * Used to resolve module chunk IDs to stable chunk names during parsing.
 * @param {Object} stats - Raw webpack stats
 * @returns {Object.<number, string>}
 */
function buildChunkIdToNameMap(stats) {
  const map = {};
  if (!stats.chunks || !Array.isArray(stats.chunks)) return map;

  for (const chunk of stats.chunks) {
    if (chunk.id != null && Array.isArray(chunk.names) && chunk.names.length > 0) {
      map[chunk.id] = chunk.names[0]; // Use first name (primary chunk name)
    }
  }
  return map;
}
...
function findDuplicatePackages(modules) {
  ...
  for (const mod of modules) {
    ...
    // Derive canonical package name from the LAST node_modules segment in installPath
    // extractPackageName uses the first node_modules/ match which is wrong for nested paths
    const nameMatch = installPath.match(/node_modules\/(?:@[^/]+\/[^/]+|[^/]+)$/);
    if (!nameMatch) continue;
    const pkgName = nameMatch[0].replace(/^node_modules\//, '');
...
module.exports = {
  ...
  buildChunkNameToAssetsMap,
  buildChunkIdToNameMap,
